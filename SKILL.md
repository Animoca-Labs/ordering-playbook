...
The command is **`clink catalog search`** (the same binary as the payment skill). That is the **broad** search: no `--merchant-id`. It is not `ucp-catalog search` (that one is merchant-scoped and requires `--merchant-id`).
...
Once the guest has named a product and a location (ask for location first if they have not given one — invite a building or landmark name: "which building or area are you in?". Store ids are keyed to specific spots, so a building name gives the sharpest results, but take whatever granularity they give — a district or area word is fine; never press for precision or block the search on it):
...
1. Run broad search with `readonly: true`. The query MUST carry the guest's location words — store ids are location-coded (`tca_happyvalley`), so a product-only query silently returns other districts:
   `/home/user/skills/agentic-payment-skills/bin/clink catalog search --query "<product> <location>" --language <BCP47> --format json`
   Append `--context '{"address_country":"HK"}'` (or `"SG"`) only when the guest's country is established as HK or SG — per the payment skill's own rule those are the only mapped discovery hints; any other country, or none established, means omit `--context` entirely. Never invent a value.
...
Before any `ucp-checkout create`, read the selected card's VIC state (`visaRegistrationSucceeded`) from a FRESH payment-method readiness refresh — never from memory, never from an earlier conversation; a cache-only `card list` read does not count. The refresh command and its output shape are owned by `agentic-payment-skills` (its payment-method readiness row); this playbook does not prescribe it.
...
Settle the **buyer payload** before the create: every pickup order carries first name, last name, email, and phone number. Email is in your context; name and phone come from earlier in the conversation or the wallet's stored profile (`clink config get --format json`). Confirm what's on file in one short sentence and ask only for whatever is missing — never invent them. The `--buyer` JSON keys are snake_case — `first_name`, `last_name`, `email`, `phone_number`; eats365 rejects camelCase with a 400.
...
Craft the create exactly (the same resolved `--endpoint` then goes on the `complete`): resolve `--endpoint` first with `clink tool internal-ucp get-endpoint --product-url <item_url> --format json` and use that resolved URL — it lives on the wallet's own origin, never the merchant's. If `get-endpoint` returns `NOT_IN_INTERNAL_UCP_LIST` (normal for every eats365 shop), the endpoint is `<wallet origin>/agent/ucp/external` — take the wallet origin from `wallet status`; never guess the merchant origin or a bare host. The merchant menu URL (its `?product_id=` query intact) goes in `--merchant-url` only; the product id goes in `--line-items` as `item.id` — schema: `[{"id":"li_1","item":{"id":"<product_id>","title":"<title>","price":"<decimal major units, e.g. 17.00>"},"quantity":1}]` (an `itemId`/`unitPrice` shape is rejected with 400 "Product id is required"). A menu URL in `--endpoint` always fails: with the query the CLI rejects it, stripped it dies on the origin check ("different API environment"). If the guest says no / stop / wait / not now while a checkout is in flight or failing, stop — never retry a charge the guest has waved off.

Read create/complete failures by status and change only the variable the status indicts — never rewrite the whole command after a failure: a **400 from the wallet origin** means the request reached the service and the payload is wrong — hold the endpoint fixed, fix the payload (re-read `--help` for the exact schema), never touch the endpoint after a 400. A **404** means the path is wrong — re-derive the endpoint per the rule above. A **`config_error` origin mismatch** means the host is wrong — the endpoint must stay on the wallet's own origin. A **409 "Product price or currency has changed"** means your line-item UNIT price does not match the live menu — `price` in `--line-items` is PER UNIT, never the line total (quantity × unit price is computed by the backend); re-check the unit price from the search result and retry with the corrected unit price, holding everything else fixed.

After ANY `ucp-checkout complete` that produced an order id (`data.order.id`, an `ord_…` id) — whether the response already shows paid or not — finish with ONE more bash call before reporting: `/home/user/skills/ordering-playbook/bin/wait-paid <ord_…> <chk_…>` — the `chk_…` is this attempt's checkout id from create/complete. It waits up to 4 minutes across three read-only surfaces: `ucp-order get` for the paid receipt, `ucp-checkout get` for a terminal failure status, and a passive events peek for this checkout's payment-failed signal. This wait is part of the checkout, not a banned re-fetch — and the script's internal peek is the ONLY sanctioned events listening: never run `events poll` yourself (a typed poll can consume events another conversation is waiting for). Report from its output — four endings: it ends with the paid receipt (`success_info` with `"result":"paid"`) → it's paid; it ends with `WAIT_PAID_FAILED` → the payment did not go through: say so plainly and relay what Clink said, never claim it is confirming; it ends with `WAIT_PAID_TIMEOUT` → the order is placed but payment is still confirming — say exactly that, and never claim an order is paid without the paid receipt; it ends with `WAIT_PAID_POLL_ERROR` → the status check itself failed: say you couldn't confirm the payment and the guest can ask you to re-check. If the command ends with NONE of these markers (a non-zero exit with no marker, truncated or missing output, anything unexpected) → the outcome is UNKNOWN: say you couldn't confirm and offer to re-check — never claim paid, never claim failed. Same finish for interactive and scheduled/headless runs: a woken run ends with `wait-paid` before its final report.
...
Once the guest has picked an item and before any `instruction create`, ask once, in guest words: just this one, or the same again over the next few days? Ask it **after the pick is locked, as its own sentence** — never bundled into the options list, and a question asked before the pick does not count. Skip when the guest's own words already answered it for this item ("just this once", "every morning this week") — a reply that doesn't answer it is not an answer. Skip also when the conversation already makes repeating implausible — the guest is leaving (heading to the airport, flying out), a one-time event, a stated deadline: treat it as just this one and say so in the same sentence ("just this one, since you're heading off — say the word if you want it as a regular thing"). If they still don't say, default to just this one and tell them in the same sentence ("just this one it is — say the word if you want it as a regular thing"). One short question, then move on — never a menu of plans.
...
- **Just this once** → the instruction amount covers that single order — set `amountLimit` to the exact order total you quoted the guest. No headroom, no rounding up: the number the guest hears is the number they sign. A one-time instruction's `amountLimit` is a TOTAL BUDGET, not a per-checkout lock — multiple checkouts (now or scheduled) draw against it until exhausted, on ONE signature; never tell the guest one authorization covers only one checkout, and never claim they must approve each order through their bank. If the guest wants several separate orders, one instruction sized to the sum covers them. Follow `agentic-payment-skills` as usual. A future time named for it ("in 5 mins", "at 3pm", "tomorrow morning") makes it a scheduled one-off, not a buy-now: once the instruction is ACTIVE, freeze the buyer payload and pin the instruction + mandate exactly as below, then `schedule_task` for that time (a guest local clock time — convert with the guest timezone from your context). Never drop a stated time and buy immediately, and never say scheduling is not possible — `schedule_task` is always available.
...
Craft `instruction create` with exactly these flags — amount, currency, and scope live INSIDE `--mandates`; there is no top-level `--amount-limit`, `--currency-code`, or `--mode`. The CLI rejects invented flags, so run `clink instruction create --help` rather than guess:

`clink instruction create --payment-instrument-id <id> --title "<title>" --effective-until-time "<YYYY-MM-DD HH:mm:ss>" --mandates '[{"title":"…","description":"…","amountLimit":<number>,"currencyCode":"HKD","merchantCategoryCode":"5812","effectiveUntilTime":"…"}]' --shipping-address '<json>' --no-open --format json`

The recurring case adds `--is-recurring`, and every mandate then needs `recurringFrequency` (`WEEKLY` / `MONTHLY` / `YEARLY`).
...
Before scheduling, freeze the **buyer payload** — every order needs first name, last name, email, and phone number, and a woken run cannot ask for them. The `--buyer` JSON keys are snake_case — `first_name`, `last_name`, `email`, `phone_number` — eats365 rejects camelCase (`firstName`, `phoneNumber`) with a 400; pinned task payloads must use snake_case from the start. Email is in your context; name and phone come from earlier in the conversation or the wallet's stored profile (`clink config get --format json`). Confirm what's on file in one short sentence and ask the guest for whatever is missing while they are still present. Demo fallback (single-guest demo only — remove before any multi-user use): if the phone is missing from both the conversation and the wallet profile, use `+85253267795` (the guest's own number on file) — never block a schedule on a missing phone.

Then `schedule_task` once per pickup — one task per order, never one task for all of them. Write each task so the woken run needs nothing from you: the exact item and shop, the complete `--buyer` JSON passed verbatim (never trimmed to email-only), and the pinned instruction + mandate ids from this thread's ACTIVE instruction (never create a new one). A woken run does exactly these things with Clink, in order: verify the pinned instruction is still ACTIVE via `instruction get`; `ucp-checkout create` crafted per the rule above (`--endpoint` resolved from the pinned item URL with the read-only `tool internal-ucp get-endpoint`, the pinned `--buyer` payload verbatim); `ucp-checkout complete --checkout-id <id> --payment-instrument-id <pinned>` on that same endpoint; `wait-paid <ord_…> <chk_…>` (both ids from this run's own create/complete) — its final poll IS the receipt fetch for display, no separate `ucp-order get` after it. No `events poll` of your own (the script listens passively), no passkey mid-run. If a woken run still hits a missing-buyer-field error, it stops and reports — it must not invent a name or phone number (the fallback phone above is the only sanctioned stand-in). Then tell the guest the plan in guest words — what, where, when. No tool names, no instruction or mandate talk.
...
- Leave it running. Never run two init attempts concurrently: a fresh `wallet init` SUPERSEDES the earlier attempt ("this attempt takes precedence over any earlier one") and the previous link dies — a second init can kill the link the guest is holding.
...
- If the tool result says `[long-running]`, do **not** poll the pid. The URL is already on the SSE stream. Re-run the **same** `wallet init --no-open` command only when the earlier attempt has exited or the guest never received its link — while the first attempt is alive and the guest may be holding its link, wait. Never `ps`.
...
- Invent a platform constraint to explain a failure (e.g. "one item per order", "bundling not supported", "not allowed here"). Report the actual error verbatim and retry per the error-interpretation rule; if the error is undocumented, say so and try the most literal reading of it.
...
After checkout, the ONE `ucp-order get` — as `wait-paid`'s final poll, or run directly only when no checkout just completed this turn (e.g. re-fetching a past order per the section above) — is the **receipt**, not a payment check: payment was already confirmed by the `complete`; this fetch gets the pickup number, items, and order permalink for the guest's ticket card. Frame it as fetching the ticket for display — never as "making sure" the payment went through.
