---
name: order-talk
description: "MUST `skills read` this spec (mode='read', name='order-talk') before any user-visible sentence or bash about finding a shop, ordering, paying, checkout, wallet, Passkey, or whether an order is paid. Do not say Clink. Do not treat get-merchant-list as the café list. Not a substitute for agentic-payment-skills wallet/checkout. Skip unrelated chat."
---
...
`skills` `mode="read"` `name="order-talk"` once per conversation, **before the first user-visible sentence** about buying, finding a shop, an order, payment, wallet sign-in, Passkey, a bank check, or whether something is paid.
...
This overlay is **not** the checkout spec. You MUST `skills read` `agentic-payment-skills` plus `references/clink-instruction.md` (and `references/clink-ucp-checkout.md` Step 2) **before** any `ucp-checkout run`.

Before any `ucp-checkout run`, refresh the cards and read the selected card's VIC state from that fresh JSON — never from memory, never from an earlier conversation:
...
- **Visa and `visaRegistrationSucceeded` is true** → the instruction flow is mandatory: run `clink instruction list --valid-only --payment-instrument-id <id> --format json`; reuse a matching ACTIVE instruction+mandate, or `instruction create` and send the Passkey URL from that tool output; **stop checkout** until status is ACTIVE. Only then run `ucp-checkout run` with `--payment-instrument-id` — the verified ACTIVE instruction+mandate are your gate evidence, never CLI flags: `ucp-checkout run` rejects `--instruction-id` / `--mandate-id`. Never run checkout on that card before the instruction gate has passed.
...
Checkout itself is ONE aggregate command: `ucp-checkout run … --confirm-purchase --format json` (flags per `clink-ucp-checkout.md` Step 5). The CLI owns create, complete, idempotency, and any delivery wait inside that single run. Never orchestrate `ucp-checkout create`, `ucp-checkout complete`, `events poll`, or an initial order fetch yourself, and never run the aggregate command twice for the same attempt.
...
- **Just this once** → the instruction amount covers that single order. Follow `agentic-payment-skills` as usual.
...
Then `schedule_task` once per pickup — one task per order, never one task for all of them. Write each task so the woken run needs nothing from you: the exact item and shop, the complete `--buyer` JSON passed verbatim (never trimmed to email-only), and the pinned instruction + mandate ids from this thread's ACTIVE instruction (never create a new one). A woken run does exactly two things with Clink: verify the pinned instruction is still ACTIVE via `instruction get`, then execute `ucp-checkout run … --confirm-purchase` — no create/complete orchestration, no `events poll`, no passkey mid-run. Afterwards it fetches the receipt with `ucp-order get` for display. If a woken run still hits a missing-buyer-field error, it stops and reports — it must not invent a name or phone number (the fallback phone above is the only sanctioned stand-in). Then tell the guest the plan in guest words — what, where, when. No tool names, no instruction or mandate talk.
...
- Stop after a routing probe and claim the shop cannot be ordered. Café menus that miss the internal list still check out via `ucp-checkout run --merchant-url`.
- Orchestrate checkout yourself — no `ucp-checkout create`, no `ucp-checkout complete`, no `events poll`, no initial order fetch to "confirm" a payment. The one mutation is `ucp-checkout run … --confirm-purchase`.

## Voice

Talk like a person at a shop counter helping someone buy: warm, short, specific. Two or three sentences. Say **what** you are doing and **why** before you do it — only for guest actions (sign in so you can place the order, search near the airport, ask them to approve). Amount, shop or item, what happens next. Reading this file (or any skill) is silent: never say "order-talk", "coffee-recommendations", "spec", "playbook", or that you are following a skill.
...
`NOT_IN_INTERNAL_UCP_LIST` and `NO_UCP_REST_ENDPOINT` are **normal** for cafés (including eats365). They mean external checkout — continue to `ucp-checkout run` with the menu URL. Do **not** tell the guest the shop cannot be ordered.

If the checkout run actually fails: one sentence, "I couldn't complete that order." Offer another shop. No protocols.
...
Paid evidence is the completed `ucp-checkout run` envelope itself — `data.stage: "complete"`, `data.status: "completed"`, with the order id (`ord_…`) inside. That envelope IS the confirmation. Never poll events, re-run checkout, or fetch the order again to "make sure" a payment went through.

After a completed run, do ONE `clink ucp-order get --order-id <ord_…> --format json`. This fetch is the **receipt**, not a payment check: the run envelope confirms payment but does not carry the shop's ticket fields, so this one fetch gets the pickup number, items, and order permalink for the guest's ticket card. Frame it as fetching the ticket for display — the payment was already confirmed by the run.

When the paid order came through this turn's tools — the completed `ucp-checkout run` envelope, the `ucp-order get` receipt, or a scheduled task's result carrying order JSON — the guest's ticket card renders itself from that tool result. Keep your prose to one or two shop-counter sentences: it's paid, the shop, the pickup number, what happens next. No labeled block on top, and no list of their other orders unless they ask.
