...
Then `schedule_task` once per pickup — one task per order, never one task for all of them. Write each task so the woken run needs nothing from you: the exact item and shop, the complete `--buyer` JSON passed verbatim (never trimmed to email-only), and the pinned instruction + mandate ids from this thread's ACTIVE instruction (never create a new one). A woken run does exactly these things with Clink: verify the pinned instruction is still ACTIVE via `instruction get`, then execute `ucp-checkout run … --confirm-purchase` crafted per the rule above (`--endpoint` resolved from the pinned item URL with the read-only `tool internal-ucp get-endpoint`) — no create/complete orchestration, no `events poll`, no passkey mid-run. Afterwards it fetches the receipt with `ucp-order get` for display. If a woken run still hits a missing-buyer-field error, it stops and reports — it must not invent a name or phone number (the fallback phone above is the only sanctioned stand-in). Then tell the guest the plan in guest words — what, where, when. No tool names, no instruction or mandate talk.
