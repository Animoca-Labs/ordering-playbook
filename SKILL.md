...
After ANY `ucp-checkout complete` that produced an order id (`data.order.id`, an `ord_…` id) — whether the response already shows paid or not — finish with ONE more bash call before reporting: `/home/user/skills/ordering-playbook/bin/wait-paid <ord_…>`. It polls `ucp-order get` for up to 5 minutes and prints the paid receipt; this wait is part of the checkout, not a banned re-fetch. Report from its output — three endings: it ends with the paid receipt (`success_info` with `"result":"paid"`) → it's paid; it ends with `WAIT_PAID_FAILED` → the payment did not go through: say so plainly and relay what Clink said, never claim it is confirming; it ends with `WAIT_PAID_TIMEOUT` → the order is placed but payment is still confirming — say exactly that, and never claim an order is paid without the paid receipt; it ends with `WAIT_PAID_POLL_ERROR` → the status check itself failed: say you couldn't confirm the payment and the guest can ask you to re-check. Same finish for interactive and scheduled/headless runs: a woken run ends with `wait-paid` before its final report.
